...
public class PersonRestController {
  ...
  public ResponseEntity<Person> getPersonById(@PathVariable Long id) {
    ...
    if (person.isPresent()) {
      log.info("Person with id {} was found and show in body response", id);
  ...
  @RequestBody Person personToAdd) {
    ...
    log.info("POST /person: Creation of Person {} sucessed with the locationId {}",
             savedPerson, locationUri.getPath());
  ...
  public ResponseEntity<Person> putPerson(@PathVariable Long id,
    ...
    if (personToUpdate.isPresent()) {
      ...
      if (updatedPerson.getFirstName().equals(currentPerson.getFirstName())
          && updatedPerson.getLastName().equals(currentPerson.getLastName())) {
  ...
  public ResponseEntity deletePerson(@PathVariable String lastName,
    ...
    Optional<Person> personToDelete =
        personService.getPersonByNames(firstName, lastName);

    if (personToDelete.isPresent()) {
      personService.deletePerson(personToDelete.get());
      log.info("DELETE \"/person/{lastName}/{firstName}\" : Person with lastName: {} and firstName: {} was successed",
               lastName, firstName);
      ...
      // Found",
      // lastName, firstName);
      throw new PersonNotFoundException("DELETE \"/person/{lastName}/{firstName}\" :Deleting Person with lastName: "
